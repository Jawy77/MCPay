 ...
 * Capabilities used: EVM read, EVM write (report), HTTP fetch, Consensus, Confidential HTTP
 */
...
const configSchema = z.object({
  // Trigger
  httpTriggerPath: z.string().default("/shield-verify"),
...
function initWorkflow(config: Config) {
  ...
  // HTTP Trigger: Agent sends verification request
  const httpTrigger = new cre.triggers.HTTPTrigger({
    path: config.httpTriggerPath,
  });

  // Main callback: orchestrate the full verify-pay-attest cycle
  const shieldVerifyCallback = (runtime: Runtime<Config>) => {
    
    // Parse the incoming request
    const rawPayload = runtime.triggerEvent.payload;
    ...
    const checkPolicySelector = "0x6a5e2650"; // keccak256("checkPolicy(address,uint256)")[:4]
    
    const policyResult = evmClient.callContract(runtime, {
      call: encodeCallMsg({
        from: "0x0000000000000000000000000000000000000000",
        to: config.shieldVaultAddress,
        data: checkPolicySelector + checkPolicyData.slice(2),
      }),
    ...
    const paymentVerification = httpClient.fetch(runtime, {
    ...
    const mcpResponse = httpClient.fetch(runtime, {
      url: config.mcpServerUrl,
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "X-ShieldPay-Attestation": "pending", // Signal to MCP we will attest
      },
      body: JSON.stringify({
        jsonrpc: "2.0",
        method: "tools/call",
    ...
    const mcpData = JSON.parse(new TextDecoder().decode(mcpResponse.body));
    ...
    serviceHash = keccak256(toBytes(contentText)) as `0x${string}`;
    ...
    const report = runtime.report(attestationData);
    ...
    runtime.log(`[ShieldPay] Attestation written! TX: ${writeResult.transactionHash}`);

    // Return result to agent
    const result = {
      success: true,
      qualityScore,
      attestationTx: writeResult.transactionHash,
      serviceHash,
      paymentHash,
